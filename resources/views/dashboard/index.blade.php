...
<form method="GET" action="{{ route('dashboard') }}" class="card mb-6 space-y-5">
    ...
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div>
            ...
            <select class="form-input min-h-32" name="branch_ids[]" multiple>
                @foreach($filterData['branches'] ?? [] as $branch)
                    <option value="{{ $branch->id }}" @selected(in_array($branch->id, array_map('intval', $filters['branch_ids'] ?? []), true))>{{ $branch->name }}</option>
                @endforeach
            </select>
            <p class="mt-1 text-xs text-slate-500">{{ __('common.multi_select_hint') }}</p>
        </div>
