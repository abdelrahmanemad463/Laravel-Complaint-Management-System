...
(() => {
    ...
    // Cross-window install marker: the installed app window and the browser tab
    // share localStorage on the same origin, so once either side knows the app
    // is installed the browser tab can hide the buttons too.
    const store = {
        installed: (() => {
            try { return window.localStorage.getItem('complaint-installed') === '1'; }
            catch (e) { return false; }
        })(),
        setInstalled() {
            try { window.localStorage.setItem('complaint-installed', '1'); } catch (e) {}
            this.installed = true;
        },
        clearInstalled() {
            try { window.localStorage.removeItem('complaint-installed'); } catch (e) {}
            this.installed = false;
        },
    };

    // Installed app window (standalone / window-controls-overlay / minimal-ui):
    // remember the install (so the browser tab hides the button too) and never
    // render install UI in the app window. CSS additionally hard-hides the
    // buttons in these display modes (see app.css), so even if this JS is stale
    // in a cached client the button cannot appear.
    if (isStandalone) {
        store.setInstalled();
        return;
    }

    // Browser tab while the app is installed: hide the buttons immediately.
    ...
    const triggerInstall = (event) => {
        ...
        deferredPrompt.userChoice.then((choiceResult) => {
            ...
            hideInstallButton();
            if (choiceResult.outcome === 'accepted') store.setInstalled();
        });
    };
    ...
    const hideInstallButton = () => {
        ...
        desktopBtn && (desktopBtn.style.display = 'none');
    ...
    // Never show the Install button once the app is already installed:
    //  1. localStorage marker set by the installed app window / appinstalled;
    //  2. getInstalledRelatedApps (secure contexts like localhost/HTTPS);
    //  3. browsers without that API (or LAN plain HTTP) trust the marker and
    ...
    if (typeof navigator.getInstalledRelatedApps === 'function') {
        navigator.getInstalledRelatedApps()
            ...
            .catch(() => { if (!store.installed) initInstallUi(); });
    } else if (!store.installed) {
        initInstallUi();
    }
})();
