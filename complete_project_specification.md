...
> **Replaced 2026-09-06** (see the violation-lifecycle entry in `memory.md`): the report page is now **immutable read-only**. The per-row Close/Update buttons and routes were removed together with the dead `VisitorCapaController`. The full lifecycle (open → in_progress → pending_review → closed / rejected → in_progress) with resolution evidence, `visit.review` approve/reject, and the no-duplicate Existing Open Violation follow-up panel is implemented by `VisitorViolationController` (`/visitors/violations*` routes) + `CapaService::submitResolution/approve/reject`; historical reports stay byte-identical because they read only snapshots and stored action status.
