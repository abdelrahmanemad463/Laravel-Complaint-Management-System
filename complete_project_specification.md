...
> **Superseded 2026-09-08** (see 66.77): the footer is now attribution-only — the contact name, phone, and LinkedIn/Facebook social links were all removed. It renders only the localized copyright line `© {year} Sultan Ayub. All rights reserved. Managed by IT Department.` centered in the shared layout.
...
- `Visitor*` models, `app/Services/Visitors/*` services (visit scoring, submission, CAPA, photo compression, checklist import), controllers under `app/Http/Controllers/Visitors`, `VisitorVisitPolicy`, `visitors.` routes, visitors seeders, bilingual `visitors.php` dictionaries, and `visitors/` Blade views with vanilla-JS autosave and progress. *(2026-09-02: the live score was removed from the inspection page — score now only shows on reports/PDF/dashboard.)*
- Rules: items start `pending` with `visited_at` null (nothing chosen by default — the inspector must actively pick Compliant/Non-compliant/Not Applicable per item); progress from `visited_at`; the score is hidden on the inspection page and computed only on the backend for reports (available minus NC deduction over non-NA items) with blue/green/yellow/red classes; owner-only in-progress access; photos required for critical/photo-required NC items and served through an authenticated route; submission is transactional, rejects unreviewed items or missing required photos, creates CAPA from NC items, and sets `completed`/`completed_at`.
...
- **Individual report** (`visitors/reports/{visit}`): QHSE-style report with header, visit info, score summary (including Score Summary due cards: Total/Critical Violations, Open Actions, Due Soon, Overdue, Immediate, Closed, Closed Late), non-compliances by severity, performance by section, root-cause analysis, violation details with Period/Due Status columns and evidence thumbnails/lightbox, and an action plan of CAPA records with remaining/overdue time and Period/Due Status columns. All historical values come from the immutable `visitors_visit_items` snapshot columns (`item_code`, `item_title`, `section_name`, `severity`, `deduction_score`, `period_hours`, immediate/corrective/preventive actions, responsible), never the master checklist.
- **Analytics dashboard** (`visitors/reports/dashboard`): summary cards, severity/section/root-cause distributions, per-branch weighted score comparison plus best/worst branches, recurring and critical violations, CAPA status analytics with overdue splitting on `due_at` and average time-to-close (computed in PHP for cross-DB compatibility), inspector performance, a day/week/month score-trend chart, and a **Corrective Actions / Due Dates** section with dueCards (open/upcoming/dueSoon/overdue/immediate/closed/closedLate/completionRate), a due-status doughnut chart (`canvas report-dueStatus-chart`, `addChart('dueStatus')` in `app.js`), and a per-branch open/overdue analysis table. Aggregations use SQL joins/groupBy/HAVING rather than loading rows into PHP.
...
- **Reuse**: `VisitScoreService` stays the single source of score truth (`fromAggregates()` now used by `calculate()`, the list, and charts); `app/Services/Visitors/DueDateService.php` is the single source of truth for all due statuses (`periodToHours`, `dueDate`, `hoursLabel`, `dueStatus`); `VisitorCapaAction::effectiveStatus()`, `dueStatus()`, and `periodLabel()` delegate to `DueDateService`.
...
- **REVERSED 2026-09-02:** this 66.66 no-default behavior was briefly replaced by a Compliant-by-default change (66.72), then reverted back to no-default the same day per user clarification (see 66.72 final state). The current behavior is **no default choice** as described here, plus the live score hidden on the inspection page. Records 66.66/66.72 are retained for history.
...
> **Replaced 2026-09-06 / 2026-09-07** (see the violation-lifecycle entry 66.75 and the resolution-review entry 66.76 in `memory.md`): the report page is now **immutable read-only**. The per-row Close/Update buttons and routes were removed together with the dead `VisitorCapaController`. The full lifecycle (open → in_progress → pending_review → closed / rejected → in_progress) with resolution evidence and the no-duplicate Existing Open Violation follow-up panel is implemented by `VisitorViolationController` (`/visitors/violations*` routes) + `CapaService::submitResolution/approve/reject`. Approval/rejection was originally gated by the coarse `visit.review` permission; on 2026-09-07 it was replaced by the separated `visit.resolution.{submit,review,approve,reject}` permissions with self-approval prevention. Historical reports stay byte-identical because they read only snapshots and stored action status.
...
## 66.70 Dashboard permissions + default home page - 2026-09-02

Added the two missing dashboard permissions and a per-user default landing page so visitors-only users no longer hit 401/403 on login. **Permissions:** `dashboard.view` (GET `/` complaint dashboard) and `dashboard.visitors.view` (GET `/visitors/reports/dashboard` visitors dashboard), seeded and translated (`permissions.dashboard.view` / `permissions.dashboard.visitors.view` / `group_dashboard`). **User default home:** new nullable `users.default_home` (`2026_09_02_000001`) + `User` fillable + `UserController` validation `in:dashboard,visitors.dashboard,complaints,visitors` + `users/form.blade.php` selector (`common.default_home` / `default_home_help` / `complaint_dashboard` / `visitors_dashboard` + `complaints` `/complaints` + `visitors` `/visitors` with URLs shown). **Routing:** `AuthController@login`/`showLogin` honors `default_home` if permitted (`complaints`→`complaint.view`, `visitors`→`visit.view`, plus the two dashboards), otherwise falls back to the first permitted dashboard (`dashboard.view` → `/`, `dashboard.visitors.view` → `/visitors/reports/dashboard`, then `complaint.view` → `/complaints`, `visit.view` → `/visitors`); an already-logged-in user hitting `GET /login` is now redirected via `redirectToHome()` instead of being able to view the login page while authenticated. **Gates:** `DashboardController` now `abort_unless(can('dashboard.view'))` (and `DashboardFilterRequest:authorize()` checks `dashboard.view`); both dashboards are STRICT — `dashboard.visitors` gate is now `dashboard.visitors.view` only (removed `report.view || visit.manage` fallback) so only roles with the dedicated permission enter the analytics dashboard, and `routes/web.php:22` adds `permission:dashboard.visitors.view` middleware on `reports.dashboard` for defense-in-depth; `visitors.reports` (list) remains `report.view || visit.manage`. `Customer Support` has `dashboard.view` only (not the visitors one), `Viewer` has both, `Admin`/`Super Admin` have all. **Navbar:** `layouts/app.blade.php:39`/`85` wraps the Dashboard links (desktop + mobile) in `@can('dashboard.view')` so the Complaint Dashboard link is hidden for users without that permission; the "Report Dashboard" button on the reports list (`visitors/reports/index.blade.php:20`) is wrapped in `@can('dashboard.visitors.view')`. Verification: `migrate` + `db:seed --class=PermissionSeeder` + `permission:cache-reset` + `view:clear` + **92 passed / 489 assertions**.

## 66.71 Admin may edit Customer Support / Viewer roles - 2026-09-02

Admin users (non-Super Admin) could not edit any of the Super Admin / Customer Support / Viewer roles - RoleController::update() blocked all three for anyone without the Super Admin role (403). The guard now restricts **only** Super Admin (`abort_unless(auth()->user()->hasRole('Super Admin'))`), so an Admin (who has `role.update`) can view and update Customer Support and Viewer permissions; editing Super Admin itself still requires the Super Admin role. `edit()` was already Super-Admin-only guarded. Verification: full suite 92 passed / 489 assertions.
## 66.72 Visitors: no default choice + hide live score on inspection page - 2026-09-02

Final state on user request (two related changes; the first briefly set Compliant-by-default then was reverted the same day). **(A) No default choice.** `VisitService::start()` creates every `visitors_visit_items` row with `status = 'pending'` and `visited_at = null` — nothing is chosen/highlighted until the inspector actively picks Compliant / Non-compliant / Not Applicable per item (status buttons render inactive on load since none match `data-status="pending"`). **(B) Live score hidden on the inspection page only.** `resources/views/visitors/show.blade.php` no longer renders the live-score header card (`visitors.live_score`, `data-live-score-value`/`data-live-final`) nor the completed-visit score-class card; the stale `$scoreColor`/`$colorClasses`/`$scoreHex` PHP block and the `applyScore()` JS function (and its `applyScore(data.score)` call in `saveItem`) were removed. The **progress** card (reviewed `N/N` + bar) remains. The score is still fully shown on the reports views (`reports/show`, `reports/index`, `reports/dashboard`, `reports/print`) — nothing there changed. `VisitController::show` still computes/passes `$score` and `VisitItemController::update` still returns it in JSON, but the inspection page ignores it, so a visit creator never sees a branch score. **Tests:** `VisitorsTest` create test back to `test_create_stores_authenticated_inspector_and_items_start_unreviewed` (all pending, 0 visited_at) and progress test back to `test_progress_based_on_visited_at` (asserts `1 / N`). Full suite: **92 passed / 489 assertions**.

## 66.73 Due-date / period_hours feature - 2026-09-04

Replaced the legacy `deadline` (string snapshot) and `due_date` columns with a `period_hours`-based due-date system across the Quality Visits module.

- **Schema:** `visitors_checklist_items.deadline` (string) and `visitors_visit_items.deadline` (string snapshot) replaced by `period_hours` (decimal 8,2, nullable; `0` = Immediate). `visitors_capa_actions` gained `period_hours` (decimal 8,2) and `due_at` (timestamp, nullable, indexed; `created_at + period_hours`, computed once at creation; `NULL` when `period_hours = 0`). Legacy `due_date` column is unused. New migration `database/migrations/2026_09_04_190000_replace_deadline_with_period_hours.php` drops `deadline` and backfills. Stored statuses = ONLY `open | in_progress | closed | rejected`; `overdue | completed | closed_late | immediate | upcoming | due_soon` are virtual (computed by `DueDateService`).
- **Config:** new `config/visitors.php` (`due_soon_hours` default `24`, env `VISITORS_DUE_SOON_HOURS`).
- **Service:** new `app/Services/Visitors/DueDateService.php` — single source of truth for `periodToHours`, `dueDate`, `hoursLabel`, `dueStatus`. `VisitorCapaAction::effectiveStatus()`, `dueStatus()`, `periodLabel()` delegate to it.
- **CAPA storage:** `CapaService::createFromNonCompliant()` stores `period_hours` + `due_at` in a DB transaction; `VisitService::start()` snapshots `period_hours` onto visit items.
- **Reports show page:** Score Summary due cards (Total/Critical Violations, Open Actions, Due Soon, Overdue, Immediate, Closed, Closed Late); Period + Due Status columns in Violation Details and the CAPA action plan (with remaining/overdue time).
- **Reports index:** gained Critical Violations / Open / Due Soon / Overdue / Immediate / Closed columns and a `due_status` filter (`open|overdue|due_soon|immediate|closed|closed_late`) backed by `VisitorReportService::listReports`.
- **Dashboard:** "Corrective Actions / Due Dates" section with dueCards (open/upcoming/dueSoon/overdue/immediate/closed/closedLate/completionRate), a due-status doughnut chart (`canvas report-dueStatus-chart`, `addChart('dueStatus')` in `app.js`), and a per-branch open/overdue analysis table.
...
## 66.75 Quality Visits violation lifecycle + follow-up (immutable reports) - 2026-09-06
...
- **Schema:** `2026_09_06_000003_extend_visitors_violation_follow_up.php` — `visitors_visit_items` += `follow_up_action` (nullable: `still_open | resolved | new_violation`) + `linked_capa_action_id` (nullable FK → `visitors_capa_actions` by `visit_item_id`, nullOnDelete); `visitors_visit_photos` += `capa_action_id` (nullable FK, nullOnDelete) + `evidence_role` (nullable: `initial | resolution`); `visitors_capa_actions` += `submitted_review_at` (nullable timestamp), `reviewed_by` (nullable FK → users, nullOnDelete), `reviewed_at`, `review_comment`, `reject_reason`. New follow-up tables `2026_09_07_000100_create_visitors_violation_follow_ups` — `visitors_violation_follow_ups` (visit, item, performer, note, result, timestamp) + `visitors_violation_follow_up_items` (link table) + `visitors_visit_photos.violation_follow_up_id`. Stored CAPA status is now **open | in_progress | pending_review | closed | rejected** (a rejected action returns to `in_progress`); virtual due statuses unchanged.
- **Lifecycle:** reports read only snapshots + stored action status, so historical reports stay byte-identical after a CAPA moves on. A follow-up resolution photo is tagged `evidence_role='resolution'` and linked to the action, leaving originating-visit photos untouched.
- **Follow-up on the inspection page** (`visitors/show.blade.php`): when a completed same-branch visit has an open action for the same `checklist_item_id`, the item renders an amber **Existing Open Violation** panel with **Still Open / Resolved / New Violation** buttons (`data-existing-violation-id`/`-status` on the `<article>`). `VisitService::submit()` routes each NC item by `follow_up_action`: `still_open` appends a `follow_up`-typed `visitors_capa_updates` row to the linked action; `resolved` calls `CapaService::submitResolution` (evidence-gated); anything else creates a new action. `ViolationFollowUpService` records non-destructive follow-up events (optional photos, multi-violation select) from the follow-up modal; **Resolved** moves the referenced violations to `pending_review` so the reviewer confirms before closing.
- **Approval/review UI:** new `VisitorViolationController` (`index` = `/visitors/violations` list with action-status filter; `show` = timeline card with initial + resolution evidence photos + `visitors_capa_updates` log + follow-up history). `approve` → `closed` (`completed_at = reviewed_at`), `reject` (requires `reject_reason`) → `in_progress`, `resolve` → `pending_review` (critical violations require an `evidence_role='resolution'` photo). Dashboard gained an amber `pending_review_capa` card (reports/dashboard cards row = 7 columns).
- **Permissions:** added `visit.review` via `PermissionSeeder`; a seeded **Quality Manager** role (15 `visit.*` permissions) granted to no user by default. Superseded 2026-09-07 by the separated `visit.resolution.*` permissions (see 66.76).
- **Verification:** full suite **136 tests / 765 assertions**; migrations + `PermissionSeeder` applied live on SQL Server; all columns/roles verified.

## 66.76 Corrective Action resolution review workflow (separated permissions, self-approval prevention) - 2026-09-07

Replaced the coarse `visit.review` gating with an explicit **submit → review (approve/reject)** workflow enforced in the backend, keeping the full audit trail and preventing a submitter from approving their own resolution.

- **Permissions (existing naming convention `{module}.{entity}.{action}`):** `visit.resolution.submit`, `visit.resolution.review`, `visit.resolution.approve`, `visit.resolution.reject`. Quality Manager: all four (legacy `visit.review` dropped from the role; the permission row still exists for compat, no code references it). Customer Support (inspectors): `visit.resolution.submit` only. Admin/Super Admin: all via `$all`. New idempotent live seeder **`ResolutionWorkflowPermissionSeeder`** (creates only the 4 perms + assigns roles + `forgetCachedPermissions()`; no password reset) for repeat deploys — run via `php artisan db:seed --class=ResolutionWorkflowPermissionSeeder --force`.
- **Schema (`2026_09_07_000200_add_resolution_review_workflow.php`):** `visitors_capa_actions` += `submitted_by` (nullable FK → users, noAction), `resolution_note` (nullable text), `closed_by` (nullable FK → users, noAction).
- **Service (`CapaService`):** `submitResolution()`/`markResolvedFromFollowUp()` now record `submitted_by = auth()->id()` + `resolution_note` and embed the resolution evidence filename in the update row; `approve()` throws `visitors.cannot_self_approve` (RuntimeException → error flash) when `submitted_by === auth()->id()` (legacy NULL rows skip the check), stores `closed_by` + `completed_at`/`reviewed_at`/`reviewed_by`, and writes an `Approved by reviewer.` update comment (closure note appended); `reject()` records `Rejected: …`.
- **Route gates (`VisitorViolationController`):** `resolve` → `visit.resolution.submit`, `approve` → `visit.resolution.approve`, `reject` → `visit.resolution.reject`.
- **Policy (`VisitorVisitPolicy::viewReport`):** additionally grants `visit.resolution.review` holders access to **any completed report** so the reviewer can evaluate other inspectors' pending resolutions; `visit.manage`/`report.view` rules unchanged.
- **UI:** `violations/show.blade.php` single Review Actions card — on `pending_review` a reviewer sees an amber submitted-resolution panel (submitter name, submission date/time, resolution note, resolution-evidence thumbnails), Approve & Close (optional closure note) gated by `visit.resolution.approve && !selfApproval` (a `cannot_self_approve` warning shows when the current user submitted it), and Reject (required reason) gated by `visit.resolution.reject`; on `open|in_progress` the resolve form (photo required only for critical) is gated by `visit.resolution.submit`. Non-reviewers on `pending_review` see `resolution_submitted_no_access`. `violations/index.blade.php` and `reports/show.blade.php` show `Submitted by: <name>` + date/time under `pending_review`; report approve/reject buttons are per-permission + self-approval guarded. `VisitorReportService` and the violation index/show eager-load `submitter`/`closer`.
- **Verification:** focused visitor test files **40 tests / 358 assertions**; full suite **158 tests / 975 assertions**. Live: `migrate --force`, `ResolutionWorkflowPermissionSeeder`, `view:clear`, `permission:cache-reset`; columns (INFORMATION_SCHEMA) and exact role grants verified.

## 66.77 Footer attribution-only revision (Sultan Ayub / IT Department) - 2026-09-08

Per the latest UI request the shared footer was reduced to a single centered copyright line; all contact and social content was removed.

- **Layout (`resources/views/layouts/app.blade.php`):** the contact block (name, phone, LinkedIn/Facebook links) is gone; the footer is now `<p>{{ __('common.footer_copyright', ['year' => now()->year]) }}</p>` centered in the shared authenticated shell.
- **Localization:** `common.footer_copyright` updated in both locales — en: `© :year Sultan Ayub. All rights reserved. Managed by IT Department.`; ar: `© :year سلطان أيوب. جميع الحقوق محفوظة. بإدارة قسم تقنية المعلومات.`. The unused contact/social keys (`footer_contact`, `footer_name`, `footer_phone(_value)`, `footer_social`, `footer_linkedin`, `footer_facebook`) remain in the dictionaries.
- **Tests:** `PwaTest::test_shared_footer_exposes_localized_attribution_without_contact_or_social_links` re-asserts the new content and the absence of `tel:`/social URLs/labels in the layout. Reported suite state: 5 PwaTest tests / 55 assertions green; full suite **158 tests / 975 assertions** (pre-footer-change runs; footer change covered by PwaTest).
- **Verification:** both `lang/en|ar/common.php` pass `php -l`, `php artisan view:clear`, PwaTest green.

## 66.78 Compliance / Complaint master data bilingual (name_en + name_ar) - 2026-09-08

Made the 7 complaint master-data entities bilingual ("name_en" + "name_ar") behind a centralized "localized_name" accessor, with safe migration of the legacy single "name" column and locale-aware display everywhere (including the visitors module's branch names). Relationships stay ID-based; user-generated content is not translated. Name-keyed business logic (dashboard solved/pending/high-critical detection, "ComplaintStatusService" solved check, customers/show + dashboard/index keyBy with Pending/In Progress/Solved/Closed, type→category+priority linkage) matches "name_en" because legacy production data is English; no new code/key column.

- **Schema (`2026_09_08_000100_make_master_data_bilingual.php`, single reversible migration):** on `branches`, `services`, `complaint_sources`, `complaint_categories`, `complaint_types`, `priorities`, `complaint_statuses` — add nullable `name_en`+`name_ar` → backfill `name_en = name`, `name_ar = name` (old value preserved in both) → drop `name` in the same migration (all code/tests deploy together). `down()` recreates `name` from `COALESCE(name_en, name_ar)` and drops the two new columns. Live SQL Server / test in-memory SQLite both verified.
- **`app/Models/Concerns/HasLocalizedName.php`:** `localized_name` → ar locale returns `name_ar ?: name_en ?: name`, else `name_en ?: name_ar ?: name` (string cast; a missing attribute is `null`, never an error). Used by all 7 models, now fillable `['name_en','name_ar',…]` (Branch also `code`; Priority `level`, `is_active`, `sort_order`; ComplaintType also `category_id`,`priority_id`).
- **Controllers/services/seeders/export:** `MasterDataController` (store/update validation requires both names; `orderBy('name_en')`); `ComplaintController::searchBranches` searches `name_en` OR `name_ar` (JSON `name` = `localized_name`); `DashboardStatsService` (selects/order/key logic on `name_en`, localized dimension/branch/filter/sum) and `ComplaintStatusService` (solved check on `name_en`, activity message `localized_name`); `MasterDataSeeder` rewritten bilingual (updateOrCreate keyed on `name_en`, branches on `code`; statuses Pending/In Progress/Solved/Closed/Reopened, priorities Low/Medium/High/Critical); `ComplaintsExport` values + status timeline use `localized_name` (`'Pending → Solved'` timeline assertion still holds under en).
- **Visitors module (branch shared with compliance):** `VisitController`, `VisitorViolationController`, `VisitorReportController` order by `name_en`; visitor blades render `$branch->localized_name`. **`VisitorReportDashboardService`** now selects `name_en`+`name_ar` (was raw `branches.name`) in `branchDueAnalysis`, `branchComparison`, and `criticalViolations`, grouped/aliased accordingly (`branch_name_en`/`branch_name_ar`) with a `pickLocalized()` helper mirroring `localized_name` — fixes SQL server would have errored on missing `branches.name`. Visitor-only master data (VisitType/Section/RootCause/Category) stays single-name (out of scope).
- **Views:** master-data `form` (two required inputs; `name_ar` gets `dir="rtl"` under en locale) + `index`; complaints `form`/`filters`/`index`/`show`; customers/show (keyBy on `name_en`); dashboard/index (keyBy `name_en`, pickers, lists); reports/branches; visitors create/show/open, violations index/show, visitor reports index/show/dashboard/print.
- **Tests:** new **`MasterDataLocalizationTest`** (store/update require both names, `localized_name` locale + fallback, Arabic master-data page, export localization — 5 tests / 18 assertions). Existing updated to `name_en`/`localized_name` throughout. Full suite **163 tests / 991 assertions** green.
- **Live deploy:** `migrate --force` (only the bilingual migration was pending), then a translation-only script (update `name_ar` for the 30 matched live rows — statuses, services, sources, categories, types, `Low/Minor/Major/Critical` priorities — **without creating/deleting**; the legacy production rows deliberately differ from the demo seeder, which would have injected Branch A/B/C and High/Medium), `view:clear`. Verified live: `name` gone, `name_en`/`name_ar` present on all 7 tables; statuses/priorities/services display Arabic under ar locale; 16 real branches fall back to their English `name_ar` under ar locale until an admin supplies Arabic via the bilingual edit form; counts unchanged (16 branches / 3 services / 6 sources / 6 categories / 6 types / 4 priorities / 5 statuses).

## 66.79 All-branches multi-select dropdown pickers - 2026-09-10

Branch multi-select pickers now show **every** branch in a scrollable panel (~7 rows visible), add an "All" select-all / clear-all option, and filter search **client-side** over the fully loaded list.

- **Server (no more 5-branch cap):** removed `->limit(5)` and the missing-branch concat from `ComplaintController::masterData()` + `searchBranches()`, `DashboardStatsService::filterData()`, `ReportController::branches()`. Because `masterData(false)` also feeds complaint create/edit, those forms now list all branches too (native `<select>`, previously capped at 5). `VisitorReportController` branch lists were already unlimited.
- **JS (`resources/js/app.js`, `data-filter-dropdown` path):** init now calls `renderBranches(allBranches)` and prepends an "All" row (`renderAllOption()`, class `branch-all`, `data-branch-all`) whose check state follows `allSelected()` (every loaded branch id present in `selectedIds`); clicking it adds every branch id (`selectedIds.add(String(...))`) or clears. Search replaced the debounced server fetch with a synchronous `filterBranches` over the full list. Legacy fallback block: removed `.slice(0, 5)` and the `:nth-child(n+6)` hiding (kept its server search).
- **Views:** `data-all-branches-label="{{ __('common.all') }}"` added to the three `data-branch-picker` containers and the branch options panel raised `max-h-56` → `max-h-64` in `complaints/filters.blade.php`, `dashboard/index.blade.php`, `reports/branches.blade.php`.
- **Tests:** `ComplaintFiltersAndAuthorizationTest` — the two picker tests now assert **all** rendered branch rows (3 seeded + 7 created = 10) and search JSON count 7 (no limit). Full suite **163 tests / 991 assertions** green.
- **Build/live:** Vite rebuilt (`public/build/assets/app-D3svSsQT.js`, verified the bundle contains the new picker code); `view:clear`. Live serves correctly (`/login` 200, unauth dashboard 302) and the 16 live branches feed all pickers (verified via live DB: `Branch::count()` = 16). No schema change, no migration.

## 66.80 Single-select branch picker on complaint create/edit - 2026-09-10

The new-complaint and edit forms (`complaints/form.blade.php`) replaced the native `<select name="branch_id">` (inside the 4-column grid, previously capped at 5 branches) with a compact **grid cell** — same 1/4-column width as Service/Source/Status — reusing the shared picker JS in a new single-select mode, searchable, scrollable (~7 rows visible). The full-width card variant was withdrawn on user feedback; the field now sits inline with the sibling selects.

- **Form markup:** a `div.relative` grid cell carries `data-branch-picker data-filter-dropdown data-single-select`; inside it a hidden `#branch_id` input (`required`, server value `old('branch_id', $complaint->branch_id ?? '')`), a `form-input` trigger with `#branch-summary` showing the chosen branch `localized_name` (or `common.select`), and the panel (`absolute start-0 end-0 top-full z-30 mt-2`, `max-h-64` scroll) containing the branch search box + all `@foreach($data['branches'])` options + Clear button. The grid loop covers `service_id/source_id/status_id` only. Edit pre-fills summary + `#branch_id` via `data-selected-id`.
- **JS (`resources/js/app.js`, `data-filter-dropdown` branch block):** new `data-single-select` mode — `syncSelectedInputs()` writes a single hidden `branch_id` instead of `branch_ids[]`; `syncButtons()` shows the selected branch name instead of a count; option click clears the set, adds the id, syncs, and **closes**; no "All" row (`if (!singleSelect) options.appendChild(renderAllOption())`); init re-adds `data-selected-id` into `selectedIds` when present. Multi-select filter/dashboard/report pickers are untouched.
- **Tests:** `ComplaintManagementTest` — `test_complaint_form_renders_single_select_branch_picker_with_all_branches` (asserts `data-single-select`, hidden `name="branch_id"`, all `Branch::count()` options, and no `data-branch-all`) and `test_complaint_edit_preselects_single_branch_in_picker` (asserts the hidden input value + visible branch name). Full suite **165 tests / 999 assertions** green.
- **Build/live:** Vite rebuilt (`public/build/assets/app-H4iHnjxH.js`, bundle verified to contain `singleSelect`); `view:cache`/`view:clear` compile cleanly; `/complaints/create` serves 302 for unauthenticated (login redirect, expected). **CSS fix:** the grid-cell panel was the first markup to use the `end-0` utility — Blade-only edits do not regenerate Tailwind output, so the panel initially shrink-wrapped wider than the trigger; a second `vite build` (CSS `app-BMezMmjn.css`, verified `.end-0` present) made the panel/search exactly match the branch field width. No schema change, no migration.

## 66.81 Create customer directly from the complaint page (quick-create modal) - 2026-09-10

The complaint create/edit form (`complaints/form.blade.php`) now lets staff create a missing customer without leaving the page: when customer search finds nothing, a localized **Create New Customer** button appears; it opens a modal with the standard customer fields, saves via a JSON endpoint reusing the existing validation, then auto-selects the new customer for the complaint.

- **Backend (`CustomerController::quickStore`, `POST customers/quick` → `customers.quick-store`):** takes the existing `StoreCustomerRequest` (same `customer.create` authorization, same name/phone/address rules, same phone trimming) so no validation is duplicated. Before creating, it checks the primary phone against **all four** phone columns via the existing `Customer::scopePhone` (exact match); on a hit it returns **422** with `customers.phone_exists` plus the existing customer payload so the UI selects it instead of duplicating. On success it creates the customer, records the standard `customer.created` activity log, and returns **201** with `customers.created` + the customer payload. No complaint is created. Framework 422 JSON (`errors` map) covers validation failures, localized through the existing `validation.php` dictionaries.
- **Localization:** `lang/{en,ar}/customers.php` += `create_title` (Create Customer / إضافة عميل), `create_new` (Create New Customer / إضافة عميل جديد), `not_found` (Customer not found. / العميل غير موجود.), `save` (Save Customer / حفظ العميل), `phone_exists`; field labels/placeholders reuse the existing `common.name/primary_phone/phone_2/phone_3/phone_4/address`, Cancel reuses `common.cancel`; `validation.php` attributes += the five customer fields in both locales (ar required message renders e.g. "حقل رقم الهاتف الأساسي مطلوب."). RTL/LTR comes from the existing `<html dir>` handling; no hardcoded Arabic in views.
- **Frontend (`resources/js/app.js`, customer-picker block):** `renderCustomers(list, term)` shows the not-found text + CTA button only when the term is non-empty with zero matches **and** the modal exists (the complaints-index filter picker has no modal, so its behavior is unchanged). Clicking the CTA opens the modal with smart prefill (phone-looking term → `phone_primary`, otherwise `name`). Save posts JSON with the page CSRF token: 201 → modal closes, customer is prepended to the local list, hidden `#customer_id` + summary update, emerald inline note; 422-with-customer → same selection path with an amber note (duplicate prevented); 422-with-errors → errors listed inside the modal, modal stays open, entered data kept; other failures → message in the modal box. Close via Cancel, overlay click, or Escape. The modal markup lives **outside** the complaint `<form>` (no nested form); Save is `type="button"` driven by JS.
- **Tests:** new `CustomerQuickCreateTest` (10 tests / 52 assertions — create+201 JSON with no complaint and activity log, optional phones empty, ar + en 422 messages with zero new rows, primary-phone duplicate returns existing, secondary-phone collision detected, `customer.create` permission 403, search-by-each-of-4-phones, create-page modal strings in both locales). Full suite **175 tests / 1051 assertions** green.
- **Build/live:** originally Vite rebuilt (`app-BD0YEK0A.js`, bundle verified to contain the modal code); `view:cache`/`view:clear` compile cleanly. No schema change, no migration, no seeder change (Customer Support already holds `customer.create`).
- **Bugfix 2026-09-10 (live):** the CTA never appeared in any browser (fresh incognito and other PCs included) because `searchCustomers` called `renderCustomers(await response.json())` **without** the search `term`, so `term` was always falsy and the not-found branch always rendered the generic empty text ("No customers found for this search.") instead of `customers.not_found` + the Create button. Fix: pass the term — `renderCustomers(await response.json(), term)` (`resources/js/app.js` line ~156; the complaints-index fallback picker intentionally stays 1-arg since it has no modal). Rebuilt to `app-CqqcEKxO.js`; served page + manifest updated; served bytes byte-match disk (SHA-256). Lesson: server rendering looks correct while the button logic lives in runtime JS — such flows need a browser-level check, not just an HTML/bundle presence scan.
