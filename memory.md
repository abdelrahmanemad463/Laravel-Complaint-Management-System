...
2026-09-10: **dashboard "New Complaint" button permission gate** — `resources/views/dashboard/index.blade.php:39` now wraps the `complaints.create` button in `@can('complaint.create')`, matching the complaints-index view (now the only two places that render it, both guarded); users without `complaint.create` no longer see a dead button on the dashboard. Tests added in `ComplaintFiltersAndAuthorizationTest` (hidden for a `dashboard.view`-only role, shown for Super Admin). Blade-only → no Vite rebuild. Full suite **179 passed / 1063 assertions**.

2026-09-10: created **`INSTALLATION-AND-SERVER-REQUIREMENTS.md`** (project root, renamed from `how-to-install-app.md`) — a from-zero install guide: PHP **8.2–8.4 (never 8.5**; phpspreadsheet caps `<8.5`), Node **20.19+/22.12+** (build-time only), mandatory PHP extensions (ctype/dom/fileinfo/filter/**gd**/hash/iconv/json/libxml/mbstring/openssl/pdo/session/simplexml/tokenizer/xml/xmlreader/xmlwriter/zip/zlib/calendar), **`sqlsrv`+`pdo_sqlsrv`** + **Microsoft ODBC Driver 17/18** at OS level, full Linux/Windows/XAMPP install steps, DB create, `.env` production baseline (sqlsrv, `SESSION_COOKIE=complaint_session`, `APP_URL` incl. path), migrate/seed commands with the **production `db:seed` warning** (PermissionSeeder `syncPermissions` overwrites real roles), web-server notes (Apache/nginx docroot=public, HTTPS for PWA), verification checklist, and troubleshooting (incl. `imagecreatefromstring()` = GD missing; permission 403s → `permission:cache-reset`). Documented authority: the same file + `memory.md`.

2026-09-10: **navbar permission gating** — `resources/views/layouts/app.blade.php` now hides **Customers** (`@can('customer.view')`), **Customer Complaints** (`@can('complaint.view')`), and the whole **Services** dropdown (`@canany(['complaint.view','visit.view'])`, per-item links also individually gated) in both the desktop navbar and mobile drawer, mirroring the exact `customer.view`/`complaint.view` abort checks used by `CustomerController@index` / `ComplaintController@index`; a user with neither permission no longer sees dead links ("This action is unauthorized"). Tests added in `ComplaintFiltersAndAuthorizationTest`: navbar hides customers/complaints/dropdown for a role with only `dashboard.view`, and shows them for Super Admin. Blade-only (no new Tailwind utility → no Vite rebuild). Full suite **177 passed / 1059 assertions**.

2026-09-10: production UI cleanup — the **demo credential hint** ("Demo: admin@example.com / password") was removed from the login page (`resources/views/auth/login.blade.php` dropped the `common.demo_login` paragraph; `demo_login` key deleted from `lang/en/common.php` and `lang/ar/common.php`, both files PHP-lint clean). No schema change, no rebuild needed (Blade/lang only, no new Tailwind utility). Production deploy = copy the two lang files + login blade (or whole app tree); DB untouched.

There are no unverified feature changes currently pending from the previous implementation work. The most recent completed work was the **create-customer-from-complaint quick-create modal** (2026-09-10, see the entry at the end of this file): when customer search on `/complaints/create` finds nothing, a localized Create New Customer button opens an in-page modal (standard fields, `common.*` labels, RTL/LTR automatic); saving posts to a new `POST customers/quick` JSON endpoint that reuses `StoreCustomerRequest` and returns 201 (auto-selects the new customer, no complaint created) or 422 (validation errors stay in-modal; primary phone colliding with any of the 4 phone columns returns the existing customer instead of duplicating). Full suite **175 passed / 1051 assertions**. Future development should first read this file and `project_structure.md`, then update both when a meaningful architectural, schema, feature, configuration, or workflow change is made.
...
### Create customer from complaint page (quick-create modal) — 2026-09-10

- **Why:** staff on `/complaints/create` had to leave the page to register a missing customer. Now a localized **Create New Customer** (`customers.create_new` / إضافة عميل جديد) button appears in the customer picker results when a non-empty search returns zero matches; it opens an in-page modal (no redirect), and saving auto-selects the new customer for the complaint without creating the complaint.
- **Backend:** `CustomerController::quickStore` (`POST customers/quick`, route `customers.quick-store`) reuses `StoreCustomerRequest` (same `customer.create` auth, same rules/trimming — Customer Support already has the permission). Duplicate guard: primary phone checked against all 4 phone columns via `Customer::scopePhone`; hit → 422 `customers.phone_exists` + existing customer payload (UI selects it); miss → create + standard `customer.created` activity log → 201 `customers.created` + payload. Validation failures use the framework 422 `errors` JSON, localized via `validation.php` (added customer attribute names in en+ar).
- **Frontend (`complaints/form.blade.php` + `resources/js/app.js` customer-picker block):** picker container carries `data-not-found-text`, `data-create-label`, `data-quick-store-url`; CTA renders only when modal markup is present (index filter picker untouched). Modal fields mirror `customers/form` with `common.*` labels/placeholders; markup sits outside the complaint `<form>` (Save is JS-driven `type="button"`). Smart prefill (phone-like term → phone field, else name); inline emerald/amber note in the picker card (no toast system exists); errors listed in-modal with data preserved; close on Cancel/overlay/Escape.
- **Tests:** `CustomerQuickCreateTest` (10 tests / 52 assertions). Full suite **175 passed / 1051 assertions**; Vite rebuilt (`app-BD0YEK0A.js`); `view:cache`/`view:clear` clean. No migration.
- **Post-launch bug (2026-09-10, root cause of "nothing shows on any device"):** `searchCustomers` called `renderCustomers(await response.json())` without the `term`, so `term` was always falsy → zero-match searches always rendered `common.no_customer_matches` ("No customers found for this search."), never the Create CTA. It was **not** a browser/server cache issue (served HTML/JS were verified byte-fresh; the bug lived in the new bundle's logic). Fixed by passing `term` (line ~156); rebuilt to **`app-CqqcEKxO.js`**, `view:clear`, verified the served page + manifest reference the new hash and served bytes match disk. Lesson: for JS-driven flows, presence of the modal/attrs in served HTML ≠ feature works; confirm the runtime branch (e.g., grep compiled `.json()` call sites for the 2-arg form).
