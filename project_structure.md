...
`ComplaintController` uses `ComplaintFilterRequest` for both the HTML index and Excel export, so exports use the same validated filters as the listing. `ComplaintController::masterData()` supplies bounded initial customer picker results (10) and the **full** active/inactive master data for the **all-branches** pickers. `CustomerController::search()` and `ComplaintController::searchBranches()` provide protected JSON endpoints consumed by the JavaScript picker code (branch search is now client-side over the fully loaded list; the endpoint remains for compatibility). The shared branch picker JS (`resources/js/app.js`, `data-branch-picker` + `data-filter-dropdown`) has two modes: **multi-select** (filters/dashboard/report, `branch_ids[]`, count summary, "All" row) and **single-select** (`data-single-select`, used by complaint create/edit where a complaint targets one branch — one hidden `branch_id`, name summary, click selects and closes, no "All" row, editable pre-fill).
...
| Customers | `CustomerController`, `Customer` model, `customers/` views | CRUD, four-phone/name search, complaint count/history, 30-record pagination, soft deletion model support; primary phone required; complaint create/edit form adds an in-page quick-create modal (`POST customers/quick` → `quickStore`, reuses `StoreCustomerRequest`, 4-column duplicate-phone guard) |
