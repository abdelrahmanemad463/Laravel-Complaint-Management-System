...
return new class extends Migration {
    public function up(): void {
        Schema::create('complaint_status_histories', function (Blueprint $table) {
            $table->id(); $table->foreignId('complaint_id')->constrained()->noActionOnDelete();
            $table->foreignId('from_status_id')->nullable()->constrained('complaint_statuses')->noActionOnDelete();
            $table->foreignId('to_status_id')->constrained('complaint_statuses')->noActionOnDelete();
            $table->text('reason')->nullable(); $table->foreignId('changed_by')->constrained('users')->noActionOnDelete();
